# +2 Rénovation — Entreprise tous corps d'état · Marcillat-en-Combraille (03)

> **Slogan** : Tous les corps d'état, une seule équipe.

+2 Rénovation E.U.R.L est une **entreprise de rénovation tous corps d'état**
basée à **Marcillat-en-Combraille** (Allier 03), depuis 2018. Avec une équipe
locale à taille humaine, nous prenons en charge votre projet de A à Z —
**de la conception à la réception des clés**, dans un rayon d'environ
20 km autour de Marcillat. Certifiée RGE QualiPAC, garantie décennale.

## Le différenciateur — un seul interlocuteur

Vous lancez une rénovation complète : toiture, isolation, fenêtres, PAC,
plâtrerie ? La plupart des artisans locaux ne couvrent qu'**un seul corps
d'état**. Vous devez alors coordonner 5 entreprises, 5 devis, 5 plannings,
5 garanties décennales différentes — et c'est vous qui assumez les conflits
de planning.

**+2 Rénovation maîtrise tous les corps d'état** et propose en plus la conception
et la coordination de projet. Un seul interlocuteur, un seul devis, un seul
planning — et la garantie décennale couvre l'ensemble.

## Pourquoi nous choisir

- **Entreprise tous corps d'état** — pas un simple artisan, une équipe.
- **Conception + coordination** — maîtrise d'œuvre globale incluse.
- **Certifications RGE & QualiPAC** — éligible MaPrimeRénov', MaPrimeRénov' Sérénité, CEE.
- **Médaille d'Or régionale** — Meilleur Apprenti de France (Thomas Buccheri).
- **20 km autour de Marcillat** — l'acteur local le plus complet.
- **Garantie décennale** + SAV joignable.
- **Devis rapide et gratuit** + visite technique offerte.

## Nos corps d'état + 1 service de coordination

### 🌟 Conception & coordination tous corps d'état
Maîtrise d'œuvre globale : étude de faisabilité, conception, planning maître,
exécution coordonnée, suivi hebdomadaire, garantie décennale couvrant
l'ensemble du projet.

### 1. Couverture & zinguerie
Réfection toiture, faîtage, gouttières et descentes en zinc. Tuiles plates,
mécaniques, ardoises. Diagnostic charpente inclus.

### 2. Menuiseries extérieures
Fenêtres, portes d'entrée, baies vitrées, volets roulants motorisés.
PVC, aluminium ou bois. Fabrication française en direct usine, profilé
VEKA, vitrage Saint-Gobain. Double et triple vitrage.

### 3. Pompe à chaleur & climatisation (RGE QualiPAC)
PAC air/eau, air/air, géothermique. Climatisation réversible mono ou
multi-split. Étude thermique offerte. Aides MaPrimeRénov' & CEE.

### 4. Placo, cloisons & isolation
Cloisons, doublages, faux plafonds, dressings. Plaques standard,
hydrofuges, phoniques, ignifuges. Doublage isolant et isolation des
combles en complément.

### 5. Maçonnerie
Création d'ouvertures, dalles, cheminées, escaliers, agrandissements,
extensions complètes. Pierre ou parpaing.

### 6. Joints de pierre
Rejointoiement à la chaux pour façades en pierre de Combraille. Mur
respirant, beauté retrouvée. Diagnostic façade offert, chiffrage précis
après visite.

### 7. VMC & chauffe-eau thermodynamique
VMC simple ou double flux haut rendement (récup 90 % calories). Ballon
thermo 200-300 L : 3× moins d'énergie qu'un cumulus.

## Réalisations récentes

Chantiers réels menés dans l'Allier (03) et le nord du Puy-de-Dôme (63) :

- **Reconstruction après incendie** — Marcillat-en-Combraille (03)
- **Réfection de toiture** (charpente + bac acier) — Marcillat-en-Combraille (03)
- **Joints de pierre à la chaux** — Marcillat-en-Combraille (03)
- **Maçonnerie & charpente** — Marcillat-en-Combraille (03)
- **Isolation intérieure** (doublage + combles) — Marcillat-en-Combraille (03)
- **Fenêtres PVC** — Roche-d'Agoux (63)
- **Menuiseries alu & volets roulants** — Saint-Maurice-près-Pionsat (63)
- **Vitrine commerciale sur mesure** — Marcillat-en-Combraille (03)
- **PAC air/eau Mitsubishi** — Marcillat-en-Combraille (03)
- **PAC air/eau** (2e installation) — Marcillat-en-Combraille (03)
- **Climatisation mono-split Gree** (salon de coiffure) — Marcillat-en-Combraille (03)
- **VMC hygroréglable** — Saint-Fargeol (03)

## Zone d'intervention

Marcillat-en-Combraille (notre base), Terjat, Mazirat, Saint-Marcel-en-Marcillat,
Ronnet, Lignerolles, Saint-Fargeol, Arpheuilles-Saint-Priest, Villebret, Hyds,
Néris-les-Bains, Montluçon.

## Avis clients (5,0 / 5 · 9 avis Google)

> « Artisan très professionnel, compétent et sérieux. Larges étendues de connaissances
> dans des domaines variés. Travail soigné, propre et réalisé dans les délais.
> Parfait, à recommander. »
> **— Sandra Jorrand (avis Google, mai 2024)**

> « Artisan très professionnel, travaux réalisés en temps et en heure, qualité du travail
> irréprochable et tarifs tout à fait acceptables. Je recommande sans aucun problème. »
> **— Pouchol-Ly Patrick (avis Google, mai 2024)**

> « Travail sérieux et appliqué, tarif respecté, résultat impeccable. À recommander. »
> **— Michel Dumery (avis Google, mai 2024)**

## Contact

- **Site** : https://www.plus2-renovation.fr/
- **Site Vercel** : https://plus2-renovation-site.vercel.app/
- **E-mail** : plus2renovation@gmail.com
- **Téléphone / WhatsApp** : 06 60 95 51 51
- **Adresse** : 8 Chemin des Bousserolles, 03420 Marcillat-en-Combraille, Allier
